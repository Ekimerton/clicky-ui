...
export default function Button({
    ...
    return (
        ...
        >
            {/* Edge layer */}
            <span
                className={`absolute inset-x-0 bottom-0 top-1.5 border-2 rounded-md border-slate-950/5 border-t-0 ${baseColor} group-active:${pressedColor} transition-all`}
            />
            {/* Front layer */}
            <span
                className={`relative flex items-center justify-center border-2 ${baseColor} rounded-md border-slate-950/5 text-gray-900 will-change-transform -translate-y-1.5 transition-all group-active:-translate-y-0 group-active:${pressedColor} ${sizeClasses}`}
            >
                {children}
            </span>
        </button>
